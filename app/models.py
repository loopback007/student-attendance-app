...
class SubjectClass(db.Model):
    ...
    students_enrolled = db.relationship('Student', secondary=enrollments,
                                        ...
                                        lazy='dynamic')
    attendance_records = db.relationship('Attendance', backref='attended_class', lazy='dynamic')
    def __repr__(self):
...
class Student(db.Model):
    ...
    # New field for arrears status
    is_in_arrears = db.Column(db.Boolean, default=False, nullable=False)

    attendance_entries = db.relationship('Attendance', backref='student_record', lazy='dynamic')
    def __repr__(self):
...
class Attendance(db.Model):
    ...
    class_id = db.Column(db.Integer, db.ForeignKey('subject_classes.id'), nullable=False)
    ...
    __table_args__ = (db.UniqueConstraint('student_id', 'class_id', 'attendance_date', name='_student_class_date_uc'),)
    recorder = db.relationship('User', backref='attendance_marked_by', foreign_keys=[recorded_by_user_id])
    def __repr__(self):
        student_name = self.student_record.full_name if self.student_record else "N/A"
        class_name = self.attended_class.name if self.attended_class else "N/A"
        return f'<Attendance {student_name} - {class_name} on {self.attendance_date}: {self.status.name}>'
